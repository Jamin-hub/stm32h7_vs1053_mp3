...
{
    spi_set_speed(SPI_BAUDRATEPRESCALER_128); /* 设置到低速模式 */
}
...
{
    spi_set_speed(SPI_BAUDRATEPRESCALER_32); /* 设置到高速模式 */
}
...
void spi_set_speed(uint32_t baudrate_prescaler)
{
    ...
    // 3. 修改 hspi1.Init 结构体中的波特率设置
    hspi1.Init.BaudRatePrescaler = baudrate_prescaler;

    // 4. 调用 HAL_SPI_Init 重新初始化（内部会配置 CR1）
